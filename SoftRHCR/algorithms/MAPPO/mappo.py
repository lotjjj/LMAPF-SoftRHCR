...
from SoftRHCR.modules.network import FeatureExtractor, Actor, Critic, MAPPOCritic
from SoftRHCR.modules.replay_buffer import PPOBuffer, _flatten_self_states
from SoftRHCR.modules.device import resolve_device, move_optimizer_state_to_device, try_load_optimizer_state
from typing import Dict, Any, List, Optional

class MAPPOAgent(AgentBase):
    def __init__(self, config, obs_info, action_dim, n_agents, n_actions=None):
        ...
        self.use_clipped_value_loss = getattr(config, "use_clipped_value_loss", True)
        ...
        self.critic_mode = getattr(config, "critic_mode", "per_agent")
        self.actor = Actor(self.feature_dim, self.n_actions).to(self.device)
        self.critic = MAPPOCritic(self.feature_dim, n_agents, mode=self.critic_mode).to(self.device)
    ...
    def _compute_values_from_features(self, features: torch.Tensor) -> torch.Tensor:
        """Compute per-agent values from extracted features (N, feature_dim).

        per_agent mode: returns (N,) — one value per agent.
        team mode: returns (N,) — team value expanded to all agents.
        """
        values = self.critic(features)      # (1, N) or (1, 1)
        values = values.squeeze(0)          # (N,) or (1,)
        if self.critic_mode == "team":
            values = values.expand(features.shape[0])  # (N,)
        return values
    ...
    def evaluate_actions(self, obs, actions, action_masks=None):
        ...
        values = self._compute_values_from_features(features)
    ...
    def update(self):
        return mappo_update(self)

    def compute_intrinsic_rewards(self, *args, **kwargs) -> Dict[str, float]:
        # MAPPO does not implement intrinsic rewards by default; users may override
